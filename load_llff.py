...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    if factor is not None:
        ...
        width = int(sh[1] / factor) # preserving ratio
        _minify(basedir, resolutions=[[height, width]])
        sfx = '_{}x{}'.format(width, height)
    elif width is not None:
        factor = sh[1] / float(width)
        height = int(sh[0] / factor) # preserving ratio
        _minify(basedir, resolutions=[[height, width]])
        sfx = '_{}x{}'.format(width, height)
    else:
        factor = 1
    ...
    if poses.shape[-1] != len(imgfiles):
        print('Mismatch between images {} and poses {} !!!'.format(len(imgfiles), poses.shape[-1]))
        return
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False):
    ...
    # Correct rotation matrix ordiering and move variable dim to axis 0
